...
$(document).ready(function() {
	...
	alert("aceess token"+ token);
	if (token != "" && token != null) {
		$(".login-signup-class").hide();
		$(".logout-class").show();
	...
	login_user = localStorage.getItem("user-email");
	...
	$(".user-login").click(function(event) {
		...
		if (flag) {
			...
			var userSignUpData = {
				"redirect_key" : redirect_key,
				"username" : user_mobile_number_or_username,
				"password" : user_password
			};
	...
	$(".small-subscribe").click(function() {
		var value = $(".small-pack").val();
		if (value > 0) {
			//	alert(value);
			callWS(value);
		}
	...
	$(".medium-subscribe").click(function() {
		var value = $(".medium-pack").val();
		if (value > 0) {
			alert(value);
			callWS(value);
		}
...
function callWS(packageId) {
	var access_token = localStorage.getItem("Access-Token");
	if (access_token != "" && access_token != null) {
		alert(packageId);
		window.location.href = "http://localhost:3005/checkouts?p="+packageId;
	} else {
